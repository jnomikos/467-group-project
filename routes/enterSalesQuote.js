...
router.post("/", (req, res) => {
    ...
    db.run(`INSERT INTO quote (customerID, employeeID, customerEmail, paymentInfo, price, description, status) VALUES ("${customerID}", "${employeeID}", "${customerEmail}", "${paymentInfo}", "${price}", "${description}", "${status}")`, (err) => {
    if (err) {
            console.log(err);
        }
